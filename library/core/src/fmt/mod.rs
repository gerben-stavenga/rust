...
//               ├────────────────────────────────┤  │   ┌───────────────────────────────┐
//   args:       │     *const Argument<'a>       0│  └─▷│ *const [FormatFn; _]          │ ─▷ […]
//               └────────────────────────────────┘      ├───────────────────────────────┤
//                                                       │ bytecode: b"\x06hello …\x00" │
//                                                       └───────────────────────────────┘
//   The descriptor is immutable static data. It starts with one pointer to the parallel formatter
//   function array, immediately followed by the byte sequence encoding the literal string pieces
//   and placeholders (including their options/flags).
//
//   The `args` pointer points to an array of one-word `fmt::Argument<'a>` values, of sufficient
//   length to match the entries in the formatter array. Placeholder entries contain only an erased
//   value pointer; dynamic width and precision entries store their integer inline.
//
//   `fmt::Arguments::new()` constructs this representation from a descriptor and an argument
//   slice. This function is unsafe, as the descriptor is assumed to contain valid bytecode and
//   matching formatter entries, and the argument slice must match both.
//
...
#[unstable(feature = "fmt_internals", issue = "none")]
impl<'a> Arguments<'a> {
    // SAFETY: The caller must ensure that the descriptor and args encode a valid fmt::Arguments,
    // as documented above, and that the descriptor remains valid for every use of the returned
    // value. The compiler-generated descriptor is stored in an inline `const`.
    #[inline]
    pub unsafe fn new<const N: usize, const M: usize>(
        descriptor: &rt::Descriptor<N>,
        args: &'a [rt::Argument<'a>; M],
    ) -> Arguments<'a> {
        // SAFETY: Responsibility of the caller.
        unsafe { Arguments { descriptor: mem::transmute(descriptor), args: mem::transmute(args) } }
    }

    // Constructs the descriptor value inside an inline `const` generated by rustc. The inferred
    // `L` parameter connects it to the corresponding static formatter array.
    #[inline]
    pub const fn new_typed_descriptor<L: rt::FormatList<M>, const N: usize, const M: usize>(
        template: &'static [u8; N],
    ) -> rt::TypedDescriptor<L, N, M> {
        rt::TypedDescriptor::new(template)
    }

    // Erases the zero-sized formatter-list type from a promoted descriptor generated by rustc.
    #[inline]
    pub fn new_descriptor<'d, L: rt::FormatList<M>, const N: usize, const M: usize>(
        _: L,
        descriptor: &'d rt::TypedDescriptor<L, N, M>,
    ) -> &'d rt::Descriptor<N> {
...
impl<'a> Arguments<'a> {
    ...
    pub fn estimated_capacity(&self) -> usize {
        ...
        // SAFETY: Non-string `Arguments` always point to a valid descriptor whose bytecode starts
        // immediately after its formatter-array pointer.
        let mut template = unsafe { self.descriptor.add(mem::size_of::<*const rt::FormatFn>()) };
        loop {
...
impl<'a> Arguments<'a> {
    ...
    pub const fn from_str(s: &'static str) -> Arguments<'a> {
        ...
        unsafe {
            Arguments {
                descriptor: mem::transmute(s.as_ptr()),
...
pub fn write(output: &mut dyn Write, fmt: Arguments<'_>) -> Result {
    ...
    // SAFETY: Non-string `Arguments` contain a valid descriptor laid out as documented above.
    let formats = unsafe { fmt.descriptor.cast::<*const rt::FormatFn>().read() };
    // SAFETY: The bytecode follows the descriptor's first pointer directly.
    let mut template = unsafe { fmt.descriptor.add(mem::size_of::<*const rt::FormatFn>()) };
    let args = fmt.args;
    ...
    loop {
        ...
        if n == 0 {
            ...
            unsafe {
                let format = formats.add(arg_index).read();
                format(
                    args.add(arg_index).as_ref().value(),
                    &mut Formatter::new(output, FormattingOptions::new()),
                )?;
            }
            arg_index += 1;
        } else {
            ...
            if n & 16 != 0 {
                ...
                // SAFETY: We can assume the template only refers to arguments that exist.
                unsafe {
                    opt.width = args.add(opt.width as usize).as_ref().as_u16();
                }
            }
            if n & 32 != 0 {
                ...
                // SAFETY: We can assume the template only refers to arguments that exist.
                unsafe {
                    opt.precision = args.add(opt.precision as usize).as_ref().as_u16();
                }
            }
            ...
            unsafe {
                let format = formats.add(arg_index).read();
                format(args.add(arg_index).as_ref().value(), &mut Formatter::new(output, opt))?;
            }
            arg_index += 1;
        }
    }
