...
/// A descriptor whose zero-sized `L` parameter ties its formatter array to the inferred marker
/// list while the descriptor is constructed in an inline `const` generated by rustc.
pub struct TypedDescriptor<L, const N: usize, const M: usize> {
    pub descriptor: Descriptor<N>,
    marker: PhantomData<fn(L)>,
}

impl<L: FormatList<M>, const N: usize, const M: usize> TypedDescriptor<L, N, M> {
    pub const fn new(template: &[u8; N]) -> Self {
        TypedDescriptor {
            descriptor: Descriptor { formats: (&L::FORMATS).as_ptr(), template: *template },
            marker: PhantomData,
        }
    }
}
